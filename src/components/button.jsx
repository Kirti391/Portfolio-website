function Button({ text, icon, variant, href, download, target, ref }) {
  const btnStyle =
    variant === "primary"
  ...
  return href ? (
    <a href={href}  download={download} className={btnStyle}  target={download ? undefined : "_blank"} rel={download ? undefined : "noopener noreferrer"} >
        {icon}
         <span>{text}</span>
    </a>
) : (
